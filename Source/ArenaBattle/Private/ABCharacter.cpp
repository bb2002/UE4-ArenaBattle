...
AABCharacter::AABCharacter()
{
	...
	CharacterStat = CreateDefaultSubobject<UABCharacterStatComponent>(TEXT("CharacterState"));
	...
	if (UIHUD.Succeeded()) {
		HPBarWidget->SetWidgetClass(UIHUD.Class);
		HPBarWidget->SetDrawSize(FVector2D(150.0F, 120.0F));
	}
	...
	if (!IsPlayerControlled()) {
		...
		if (ABGameInstance != nullptr) {
			AssetStreamingHandle = 
				ABGameInstance->StreamableManager.RequestAsyncLoad(
...
void AABCharacter::SetWeapon(AABWeapon * NewWeapon)
{
	...
	if (nullptr != CurrentWeapon)		// 이전 Weapon 을 제거 한다.
	{
...
{
	float Range = GetFinalAttackRange();
	...
	float HalfHeight = Range * 0.5f + AttackRadius;
	...
	UE_VLOG_LOCATION(this, ArenaBattle, Verbose, GetActorLocation(), 50.0F, FColor::Blue, TEXT("ATTACK POS"));
...
float AABCharacter::GetFinalAttackDamage() const {
	float AttackDmg = (nullptr != CurrentWeapon) ? (CharacterStat->GetAttack() + CurrentWeapon->GetAttackDamage()) : CharacterStat->GetAttack();
	float AttackMod = (nullptr != CurrentWeapon) ? CurrentWeapon->GetAttackModifier() : 1.0F;
	return AttackDmg * AttackMod;
}